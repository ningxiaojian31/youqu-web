						...
						<view v-for="(item, index) in invList" :key="index" class="news-item">
							<view @click="toPersonal()" style="display: flex; flex-direction: row;margin-bottom: 15upx;">
								<view class="meHeadAvatar">
									<image class="meHeadAvatar-image" v-if="item.image != null && item.image != ''" :src="item.image"></image>
									<image class="meHeadAvatar-image" v-else src="../../static/chatImage.png"></image>
								</view>
		...
		export default {
			...
			methods: {
				toPersonal(){
					var that = this;
					uni.navigateTo({
						url:'../me/personal?userId='+that.userId
					});
				},
				...
				loadInvList(type){
					...
					//请求列表
					var url = common.apiHost+'/invitation/tInvitation/list?currentPage='+reqData.currentPage+'&pageSize='+reqData.pageSize;
					var method = "POST";
					common.request(url,reqDataOther,method).then(data => {
					 console.log("请求列表成功==============");
					 //赋值
					 setTimeout(() => {
						 var record = null;
